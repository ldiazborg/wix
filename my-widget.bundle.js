(function () {
  ...
  function hsfw_getReferringPageVariables(window) {
  ...
  function hsfw_getClientInfo(window, document, navigator) {
    return encodeURIComponent(
      JSON.stringify({
        referringSiteURL: window.location.href,
        referringPageName: window.document.title,
        previousPage: document.referrer,
        referringPageVariables: hsfw_getReferringPageVariables(window),
  ...
  function hsfw_createIframe(document, formId, formattedUrl, clientInfo, window, hsfw_prod) {
  ...
  function hsfw_updateGaIframe(event, window) {
  ...
  function hsfw_executeFunction(event, window) {
  ...
  function hsfw_resizeContainer(event, document) {
  ...
  function handleMessageEvent(event, gaFirstTime, window, document) {
    ...
    if (event.data.hs_gaPageToOpen) {
      hsfw_updateGaIframe(event, window);
    }

    if (event.data.calledFunction) {
      hsfw_executeFunction(event, window);
    }

    if (event.data.hsfw_height) {
      hsfw_resizeContainer(event, document);
    }
  }
  ...
  function hsfw_addMessageListener(window, document) {
    ...
    window.addEventListener("message", (event) => {
      handleMessageEvent(event, gaFirstTime, window, document);
    });
  }

  function wix_widget_container_factory(document, window, hsfw_formId, navigator, hsfw_prod) {
    ...
    const clientInfo = hsfw_getClientInfo(window, document, navigator);
    hsfw_setContainerStyle(container);
    const iframe = hsfw_createIframe(document, hsfw_formId, formattedUrl, clientInfo, window, hsfw_prod);
  ...
  class HelloWorld extends HTMLElement {
      ...
      //     return ['formid']; // Define attributes to observe
      // }
      ...
      connectedCallback() {
          console.log("Wix Widget connectedCallback");
          var formId = this.getAttribute('formid');
          this.appendChild(wix_widget_container_factory(document, window, formId, navigator, false));
      
          // this.appendChild(wix_widget_container_factory(document, window, "4ad34e7b-76a9-4547-afab-8adeb2fb0a86", navigator, false))
      }
  }
  customElements.define('wix-widget', HelloWorld);
